...
func (session *Session) innerInsert(bean interface{}) (int64, error) {
	...
	if session.engine.dialect.DBType() == core.ORACLE && len(table.AutoIncrement) > 0 {
		...
		fmt.Printf("UseIdentity %v\n", session.statement.UseIdentity)

		if len(res) < 1 {
			if !session.statement.UseIdentity {
				return 0, errors.New("insert successfully but not returned id")
			}

			//var a []byte
			//a := make([]byte, 1)
			//res = []map[string][byte]{table.AutoIncrement:a}
			//var keyvalue []map[string]byte
			//keyvalueslice := make([]keyvalue, 1, 1)
			//res = make([]map[string][]byte, 1, 1)
			//res[0][table.AutoIncrement] = a
			//x := make([]map[string][]byte, 0)
			//x[0][table.AutoIncrement] = append(x[0][table.AutoIncrement], 0)
			//x["key"] = append(x["key"], "value1")
			m := make([]map[string][]byte, 0)
			res = append(m, map[string][]byte{table.AutoIncrement: []byte("0")})

		}

		idByte := res[0][table.AutoIncrement]
		id, err := strconv.ParseInt(string(idByte), 10, 64)
		if err != nil || (!session.statement.UseIdentity && id <= 0) {
			return 1, err
		}
